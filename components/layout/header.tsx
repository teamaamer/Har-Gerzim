...
<<<<<<< HEAD
      <div className={`w-full px-4 md:px-6 lg:px-8 max-w-7xl mx-auto flex h-20 items-center justify-between relative ${
        locale === 'en' ? 'flex-row-reverse lg:flex-row' : 'flex-row-reverse lg:flex-row'
      }`}>
=======
      <div className="w-full px-4 md:px-6 lg:px-8 max-w-7xl mx-auto flex h-20 items-center justify-between relative">
>>>>>>> 067bdc9f387a3afea5b22a5a803dba1176f21dc6
...
<<<<<<< HEAD
                <ChevronDown className={`h-3 w-3 transition-transform duration-300 ${productsMenuOpen ? 'rotate-180' : ''}`} />
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-gold-600 group-hover:w-full transition-all duration-300" />
              </button>
                ...
                {productsMenuOpen && (
                  ...
                  >
                    <motion.div 
                      ...
                      className="bg-white/90 backdrop-blur-xl border border-white/20 rounded-lg shadow-lg py-2"
                    >
                      <motion.div
...
<<<<<<< HEAD
            ...
            <SheetContent side={locale === 'he' || locale === 'ar' ? 'right' : 'left'} className="w-[300px] bg-white">
              ...
              <nav className="flex flex-col gap-4 mt-8">
                ...
                >
                  ...
                  >
                    {dict.nav.collections || 'Products'}
                    <ChevronDown className={`h-5 w-5 transition-transform duration-300 ${productsMenuOpen ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    {productsMenuOpen && (
                      <motion.div 
                      ...
                      >
                        <Link
                          href={allProductsOption.href}
                          className="block text-base font-bold text-white bg-gradient-to-r from-gold-500 to-gold-600 hover:from-gold-600 hover:to-gold-700 transition-all py-2 px-3 rounded-md shadow-md mb-2"
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {productCategories.map((category, index) => (
                            ...
                            >
                              <Link
                                href={category.href}
                                className="block text-sm font-medium text-navy-700 hover:text-gold-600 hover:bg-gold-50 transition-colors py-2 px-2 rounded-md"
                                onClick={() => setMobileMenuOpen(false)}
                              >
                                {category.label}
                              </Link>
          ...
          <div className="relative">
            ...
            <CartPreview open={cartOpen} onClose={() => setCartOpen(false)} locale={locale} dict={dict} />
          </div>
